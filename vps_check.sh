...
# - 菜单模式：System / IP / Ping / MTR / Disk / Streaming / TCP(真实链路)
# - 9  : 全跑(2~8+10)，显示全过程
# - R  : 静默全跑(2~8+10)，不刷屏，只输出最终✅总结报告
# - 10 : TCP 真实链路测试（TLS/TTFB/下载Mbps，更贴近代理体验）
# - 修复：MTR 最后一跳 loss/avg 解析更稳（不再出现 ?%/100ms 乱值）
# - 美化：最终总结报告进度条 & 评级颜色
#
# Usage:
...
#
# NOTE:
# - 只读检测，不改系统配置（除非你在菜单里选择“安装 mtr-tiny”）
...
# TCP test
TCP_MAXTIME=12
TCP_RANGE_BYTES=$((8*1024*1024))  # 8MB range
TCP_SOURCE_NAME="hetzner 100MB (range 8MB)"
TCP_SOURCE_URL="https://speed.hetzner.de/100MB.bin"
...
mask_host() {
  local h="${1:-}"
  [[ -z "$h" || "$h" == "unknown" ]] && echo "$h" && return 0
  local n="${#h}"
  if (( n <= 2 )); then echo "*"; return 0; fi
  echo "${h:0:1}***${h: -1}"
}
...
# tcp summary
TCP_TLS_MS="" TCP_TTFB_MS="" TCP_DL_MBPS="" TCP_HTTP_CODE="" TCP_RATING="unknown" TCP_SCORE=0
...
grade_color() {
  local x="$1" t
  t="$(grade_text "$x")"
  if [[ "$x" -ge 85 ]]; then echo -e "${GREEN}${t}${NC}"
  elif [[ "$x" -ge 70 ]]; then echo -e "${YELLOW}${t}${NC}"
  elif [[ "$x" -ge 55 ]]; then echo -e "${YELLOW}${t}${NC}"
  else echo -e "${RED}${t}${NC}"
  fi
}
score_badge() {
  local x="$1"
  if [[ "$x" -ge 85 ]]; then echo -e "${GREEN}✅${NC}"
  elif [[ "$x" -ge 70 ]]; then echo -e "${YELLOW}🟡${NC}"
  elif [[ "$x" -ge 55 ]]; then echo -e "${YELLOW}⚠️${NC}"
  else echo -e "${RED}❌${NC}"
  fi
}
bar() {
  ...
  local width=24
  local fill=$(( s*width/100 ))
  local empty=$(( width-fill ))

  local b_filled b_empty
  b_filled="$(printf "%0.s█" $(seq 1 "$fill" 2>/dev/null || true))"
  b_empty="$(printf "%0.s░" $(seq 1 "$empty" 2>/dev/null || true))"

  # color by grade
  if (( s>=85 )); then
    echo -e "${GREEN}[${b_filled}${b_empty}]${NC}"
  elif (( s>=70 )); then
    echo -e "${YELLOW}[${b_filled}${b_empty}]${NC}"
  elif (( s>=55 )); then
    echo -e "${YELLOW}[${b_filled}${b_empty}]${NC}"
  else
    echo -e "${RED}[${b_filled}${b_empty}]${NC}"
  fi
}
...
run_mtr() {
  ...
  # 最后一条非空行
  last_line="$(echo "$out" | awk 'NF{line=$0} END{print line}')"

  # 解析：找到第一个形如 0.0% 的字段 i，然后 avg = i+3
  # 格式列：Loss% Snt Last Avg Best Wrst StDev
  read -r last_loss last_avg < <(echo "$last_line" | awk '
    {
      loss=""; avg="";
      for(i=1;i<=NF;i++){
        if($i ~ /^[0-9.]+%$/){
...
run_tcp() {
  ...
  # 用 range 拉 8MB，不占太多流量
  local out code app ttfb spd
  out="$(curl -L -s --max-time "${TCP_MAXTIME}" \
    --range "0-$((TCP_RANGE_BYTES-1))" \
    -o /dev/null \
    -w "code=%{http_code} app=%{time_appconnect} ttfb=%{time_starttransfer} spd=%{speed_download}" \
    -A "Mozilla/5.0" \
    "${TCP_SOURCE_URL}" 2>/dev/null || true)"

  code="$(echo "$out" | sed -n 's/.*code=\([0-9]\{3\}\).*/\1/p')"
  app="$(echo "$out"  | sed -n 's/.*app=\([0-9.]*\).*/\1/p')"
  ttfb="$(echo "$out" | sed -n 's/.*ttfb=\([0-9.]*\).*/\1/p')"
  spd="$(echo "$out"  | sed -n 's/.*spd=\([0-9.]*\).*/\1/p')"

  TCP_HTTP_CODE="${code:-000}"
  TCP_TLS_MS="$(ms_from_sec "$app")"
  TCP_TTFB_MS="$(ms_from_sec "$ttfb")"

  # speed_download 是 bytes/sec -> Mbps
  spd="$(safe_num "$spd")"
  if [[ -n "${spd:-}" ]] && f_ge "$spd" "1"; then
    TCP_DL_MBPS="$(awk -v x="$spd" 'BEGIN{printf "%.0f", (x*8)/1000000}')"
  else
    TCP_DL_MBPS=""
  fi
  ...
  if [[ -n "${TCP_DL_MBPS:-}" ]]; then
    echo "Download       : ${TCP_DL_MBPS} Mbps  (source=${TCP_SOURCE_NAME}, maxtime=${TCP_MAXTIME}s, http_code=${TCP_HTTP_CODE})"
  else
    echo "Download       : ? Mbps  (source=${TCP_SOURCE_NAME}, maxtime=${TCP_MAXTIME}s, http_code=${TCP_HTTP_CODE})"
  fi

  # scoring
  local tls_s=0 ttfb_s=0 dl_s=0 w_tls=40 w_ttfb=40 w_dl=20 used=0 total=0

  # tls
  if [[ -n "${TCP_TLS_MS:-}" ]]; then
    if [[ "$TCP_TLS_MS" -lt 80 ]]; then tls_s=100
    elif [[ "$TCP_TLS_MS" -lt 150 ]]; then tls_s=80
    elif [[ "$TCP_TLS_MS" -lt 300 ]]; then tls_s=60
    else tls_s=40
    fi
  fi
  ...
  if [[ -n "${TCP_TTFB_MS:-}" ]]; then
    if [[ "$TCP_TTFB_MS" -lt 120 ]]; then ttfb_s=100
    elif [[ "$TCP_TTFB_MS" -lt 250 ]]; then ttfb_s=80
    elif [[ "$TCP_TTFB_MS" -lt 600 ]]; then ttfb_s=60
    else ttfb_s=40
    fi
  fi
  ...
  # download
  if [[ -n "${TCP_DL_MBPS:-}" ]]; then
    if [[ "$TCP_DL_MBPS" -ge 200 ]]; then dl_s=100
    elif [[ "$TCP_DL_MBPS" -ge 80 ]]; then dl_s=80
    elif [[ "$TCP_DL_MBPS" -ge 20 ]]; then dl_s=60
    else dl_s=40
    fi
  else
    # 没测速就不计入 dl 权重
  ...
  used=$((w_tls + w_ttfb + w_dl))
  if (( used > 0 )); then
    total="$(awk -v a="$tls_s" -v b="$ttfb_s" -v c="$dl_s" -v wa="$w_tls" -v wb="$w_ttfb" -v wc="$w_dl" -v u="$used" 'BEGIN{printf "%.0f", (a*wa + b*wb + c*wc)/u}')"
  else
    total=0
  fi

  TCP_SCORE="$total"
  if [[ "$TCP_SCORE" -ge 85 ]]; then TCP_RATING="GOOD"; ok "TCP 体验：优秀"
  elif [[ "$TCP_SCORE" -ge 70 ]]; then TCP_RATING="WARN"; warn "TCP 体验：良好（可用）"
  elif [[ "$TCP_SCORE" -ge 55 ]]; then TCP_RATING="WARN"; warn "TCP 体验：一般（有波动可能）"
  else TCP_RATING="BAD"; bad "TCP 体验：偏弱（不建议做高稳定落地）"
  fi
...
# ---------- overall summary ----------
overall_summary() {
  local host_show="$HOSTNAME_" ip_show="$IPV4_"
  if (( REDACT == 1 )); then
    host_show="$(mask_host "$HOSTNAME_")"
    ip_show="$(mask_ip "$IPV4_")"
  fi
  ...
  # TCP score already computed
  local tcp_score=0
  [[ "$RUN_TCP" -eq 1 ]] && tcp_score="$TCP_SCORE" || tcp_score=0
  ...
  echo -e "${PURPLE}${BOLD}====================== ✅ VPS 体检总结报告 ======================${NC}"

  echo -e "${BOLD}${BLUE}[基础信息]${NC}"
  ...
  echo -e "${BOLD}${BLUE}[网络]${NC}  $(score_badge "$net_score")  ${BOLD}${net_score}/100${NC} ($(grade_color "$net_score"))  $(bar "$net_score")"
  if [[ "$RUN_PING" -eq 1 ]]; then
  ...
  echo -e "${BOLD}${BLUE}[TCP真实链路]${NC}  $(score_badge "$tcp_score")  ${BOLD}${tcp_score}/100${NC} ($(grade_color "$tcp_score"))  $(bar "$tcp_score")"
  if [[ "$RUN_TCP" -eq 1 ]]; then
    echo " TLS  : ${TCP_TLS_MS:-?} ms | TTFB=${TCP_TTFB_MS:-?} ms"
    if [[ -n "${TCP_DL_MBPS:-}" ]]; then
      echo " DL   : ${TCP_DL_MBPS} Mbps (source=${TCP_SOURCE_NAME}, maxtime=${TCP_MAXTIME}s, http_code=${TCP_HTTP_CODE})"
    else
      echo " DL   : ? Mbps (source=${TCP_SOURCE_NAME}, maxtime=${TCP_MAXTIME}s, http_code=${TCP_HTTP_CODE})"
    fi
  ...
  echo -e "${BOLD}${BLUE}[磁盘]${NC}  $(score_badge "$disk_score")  ${BOLD}${disk_score}/100${NC} ($(grade_color "$disk_score"))  $(bar "$disk_score")"
  if [[ "$RUN_DISK" -eq 1 ]]; then
  ...
  echo -e "${BOLD}${BLUE}[流媒体]${NC}  $(score_badge "$stream_score")  ${BOLD}${stream_score}/100${NC} ($(grade_color "$stream_score"))  $(bar "$stream_score")"
  if [[ "$RUN_STREAM" -eq 1 ]]; then
  ...
  echo -e "${BOLD}${BLUE}[总评]${NC}  $(score_badge "$total")  ${BOLD}${total}/100${NC} ($(grade_color "$total"))  $(bar "$total")"
  if [[ "$total" -ge 85 ]]; then
    ok "结论：整体素质很强，适合做中转/落地/流媒体测试/轻量服务。"
  elif [[ "$total" -ge 70 ]]; then
    warn "结论：整体不错，日常中转/落地够用，建议关注路由与邻居波动。"
  ...
  echo
  if (( REDACT == 1 )); then
    info "已启用 --redact：IPv4/Host 已自动打码。"
  else
...
run_all_quiet() {
  ...
  local tmp_log
  tmp_log="$(mktemp -t vpscheck.XXXXXX)"
  {
    gather_system
    gather_ip
    run_ping_all
...
menu() {
  while true; do
    echo -e "${PURPLE}${BOLD}====================== VPS 一键体检 菜单 ======================${NC}"
    echo -e "Targets: ${BOLD}${TARGETS[*]}${NC}  ${GRAY}(MTR 默认用第一个 Target)${NC}"
    echo
